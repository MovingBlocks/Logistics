...
folder(baseModuleFolderName) {
    description("These folders are normally letter-based groupings of module jobs, but for Nanoware contain two groupings at differing maturity levels")
}
...
// Reason is too many jobs in one folder slows things down. If needed ABCmCzDE... could probably split several per letter
char[] letters = "X".toCharArray() // Smaller test
    ...
    job(seedModuleFolderNameLetter) {
        ...
        steps {
            dsl {
                ...
                def seedJobDSL = """
githubOrg = "Nanoware"
credId = "github-app-terasology-jenkins-io"
githubApiUri = "https://api.github.com"
orgPipelineRepo = "https://github.com/Nanoware/ModuleJteConfig.git"
libraryRepo = "https://github.com/Nanoware/ModuleJteLibraries.git"

organizationFolder "$baseModuleFolderName/$letter", {
    // TODO: May need to tweak the cleanup config
    description "Build jobs for Terasology modules under the test org Nanoware. **NOTE:** This job is hardwired to very few modules<br><br>This job targets the <b>experimental</b> engine and ModuleJteConfig branches"
    displayName "$letter"
        ...
        github {
            ...
            traits {
                ...
                sourceRegexFilter {
                    ...
                    // NOTE HARD CODED MODULE NAMES HERE - not a `letter + '.*'` regex or something - not even the same letter! But enough to run a few MTE tests
                    regex("Hunger|Health|CoreAssets|ModuleTestingEnvironment")
                }
            }
        }
    }
